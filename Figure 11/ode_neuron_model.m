function [time v_E v_I S_EI S_IE X_EI X_IE Apost Apre Apost_EI Apre_EI W_IE W_EI spike_E spike_I ref_E ref_I synchrony spt_E] = ode_neuron_model(plast_on,ON1,vE0,vI0,S_EI0,S_IE0,X_EI0,X_IE0,Apost0,Apre0,Apost0_EI,Apre0_EI,W_IE0,W_EI0,mew_e,sigma_e,ue,ui,mew_i,sigma_i,J_E,J_I,C_E,C_I,tau_LTP,tau_LTD,step,sample_duration,N_E,N_I,S_key_EI,S_key_IE,leftover_S_EI,leftover_S_IE,ref_E,ref_I,Energy0,tau_E_m,tau_I_m, percent_V_stim,comp_time,spt_E0)
% -- run parameters --
...
% -- intialize vectors --
    ...
    dApostdt_EI = zeros(duration_step+1,num_synapses_EI);
    dApredt_EI = zeros(duration_step+1,num_synapses_EI);
    ...
    W_EI = zeros(duration_step+1,num_synapses_EI);
    Apost_EI = zeros(duration_step+1,num_synapses_EI);
    Apre_EI = zeros(duration_step+1,num_synapses_EI); 
    spt_E = zeros(1,N_E); 
...
% --- update synaptic variables ---
    ...
    % dxdt update
        ...
        dApostdt_EI(i,:) = -Apost_EI(i,:)/tau_LTP;
        dApredt_EI(i,:) = -Apre_EI(i,:)/tau_LTD;
        
    % x update
        Apost(i+1,:) = Apost(i,:) + step*dApostdt(i,:);
        Apre(i+1,:) = Apre(i,:) + step*dApredt(i,:);
        Apost_EI(i+1,:) = Apost_EI(i,:) + step*dApostdt_EI(i,:);
        Apre_EI(i+1,:) = Apre_EI(i,:) + step*dApredt_EI(i,:);
        W_IE(i+1,:) = W_IE(i,:);
        W_EI(i+1,:) = W_EI(i,:);
%     
    ...
    for k = 1:N_E
        if v_E(i+1,k) >= 20 && v_E(i,k) < 20
            ...
            for j = 1:N_I
                ...
                if S_key_EI(j,k) ~= 0
                    index = S_key_EI(j,k);
                    Apost_EI(i+1,index) = Apost_EI(i,index) + dApost_0;
%                     W_EI(i+1,index) = W_EI(i,index) + eta*a_LTP*Apre_EI(i,index); % hebbian
                    W_EI(i+1,index) = W_EI(i,index) + eta*a_LTD*Apre_EI(i,index); % anti-hebbian
                    
                    % max synaptic weight check
                    if (J_E*W_EI(i+1,index)) > 290
                        W_EI(i+1,index) = 290/J_E;                   
                    end
                end
        ...
        if v_I(i+1,k) >= 20 && v_I(i,k) < 20
            ...
            for j = 1:N_E
                ...
                if S_key_EI(k,j) ~= 0
                    index = S_key_EI(k,j);
                    % synaptic input from E to I : _(IE)
                    X_EI(i+1,j) = X_EI(i,j) - W_EI(i,index);
                     
                    % plasticity update - "on_pre"
                    Apre_EI(i+1,index) = Apre_EI(i,index) + dApre_0;
%                     W_EI(i+1,index) = W_EI(i,index) + eta*a_LTD*Apost_EI(i,index); % hebbian
                    W_EI(i+1,index) = W_EI(i,index) + eta*a_LTP*Apost_EI(i,index); % anti-hebbian
                    
                    % max synaptic weight check                       
                    if (J_E*W_EI(i+1,index)) < 10
                        W_EI(i+1,index) = 10/J_E;
                    end
                end
