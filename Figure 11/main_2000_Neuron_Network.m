...
% -- Synaptic Parameters --
    Weight_0 = 1;
    J_E = 260; % NOTE: J_E = J_EI 100
    J_I = 260; % NOTE: J_I = J_IE 225
    N_i = 1;
    C_E = 0.3*Ntot;
    C_I = 0.3*Ntot;
    tau_LTP = 20;
...
% -- Initial Conditions --
    ...
    W_EI0 = Weight_0*ones(1,num_synapses_EI); 
    leftover_S_EI = zeros(5/step + 1 ,N_E);
    leftover_S_IE = zeros(5/step + 1 ,N_I);
    ref_E = zeros(1,N_E);
    ref_I = zeros(1,N_I);
    Apost0_EI = zeros(1,num_synapses_EI);
    Apre0_EI = zeros(1,num_synapses_EI);
    spt_E0 = 0;
    tau_E_m = 10; 
    tau_I_m = 10;
...
for i = 1:duration/sample_duration
    ...
    if mean(W_IE0)*J_I < 75 && mean(W_EI0)*J_E < 75
        cross_100 = 0;
    end
    
    % Plasticity and Control input
    ON = 1*(i*sample_duration >= 2000)*cross_100; % turn on control input
    plast_on = (i*sample_duration >= 100); % past on/off
    ...
    [timem v_Em v_Im S_EIm S_IEm X_EIm X_IEm Apostm Aprem Apost_EIm Apre_EIm W_IEm W_EIm spike_Em spike_Im ref_Em ref_Im synchronym spt_Em] = ode_neuron_model(plast_on,ON,vE0,vI0,S_EI0,S_IE0,X_EI0,X_IE0,Apost0,Apre0,Apost0_EI,Apre0_EI,W_IE0,W_EI0,mew_e,sigma_e,ue,ui,mew_i,sigma_i,J_E,J_I,C_E,C_I,tau_LTP,tau_LTD,step,sample_duration,N_E,N_I,S_key_EI,S_key_IE,leftover_S_EI,leftover_S_IE,ref_E,ref_I,Energy0,tau_E_m,tau_I_m, percent_V_stim,comp_time,spt_E0);
    ...
    W_EI(a:b,1) = mean(W_EIm(1:sample_duration/step,:),2);
...
%     spike_I(a:b,:) = spike_Im(1:sample_duration/step,:);
    ...
    Apost0_EI = Apost_EIm(sample_end,:);
    Apre0_EI = Apre_EIm(sample_end,:);
...
title('Average Synpatic Weight E to I')
ylim([0 300])
xlabel('Time (sec)')
...
subplot(2,1,2)
plot(time/1000,J_E*W_EI,'r')
title('Average Synpatic Weight I to E')
ylim([0 300])
xlabel('Time (sec)')
ylabel('Average Synpatic Weight')
xlim([0 duration/1000])
