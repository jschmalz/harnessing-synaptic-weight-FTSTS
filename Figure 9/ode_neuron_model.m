...
% -- neuron parameters --
    ...
    stim_noise_E = randn(duration_step+1,N_E); % gaussian white noise
    stim_noise_I = randn(duration_step+1,N_I); % gaussian white noise
    vreset = 14;
    refractory = 2; % ms
...
% --- Contamination
    contam_percent_E = zeros(1,N_E);
    contam_percent_E(1,1:400) = 1;
    contam_percent_I = zeros(1,N_I);
    contam_percent_I(1,1:100) = 1;
...
% -- excitatory --
        % dxdt update
        if i > syn_delay/step
            dv_Edt(i,:) = (vrest - v_E(i,:) + J_E/C_E*S_EI(i-syn_delay/step,:) + mew_e + ON1.*ue(1,i) + ON1.*ui(1,i).*contam_percent_E + sigma_e.*(tau_E_m(1,:).^0.5).*whitenoise_E(i,:))./tau_E_m(1,:);
        else
            dv_Edt(i,:) = (vrest - v_E(i,:) + J_E/C_E*leftover_S_EI(i,:) + mew_e + ON1.*ue(1,i) + ON1.*ui(1,i).*contam_percent_E + sigma_e.*(tau_E_m(1,:).^0.5).*whitenoise_E(i,:) )./tau_E_m(1,:);
        end
...
%         dxdt update
        if i > syn_delay/step
            dv_Idt(i,:) = (vrest - v_I(i,:) + J_I/C_I*S_IE(i-syn_delay/step,:) + mew_i + ON1.*ui(1,i) + ON1.*ue(1,i).*contam_percent_I + sigma_i.*(tau_I_m(1,:).^0.5).*whitenoise_I(i,:))./tau_I_m(1,:);
        else
            dv_Idt(i,:) = (vrest - v_I(i,:) + J_I/C_I*leftover_S_IE(i,:) + mew_i + ON1.*ui(1,i) + ON1.*ue(1,i).*contam_percent_I  + sigma_i.*(tau_I_m(1,:).^0.5).*whitenoise_I(i,:))./tau_I_m(1,:);
        end
